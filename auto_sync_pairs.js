...
class BinancePairSyncService {
  ...
  async start() {
    try {
      ...
      console.log(`\n🔄 Auto-sync service started - will check every ${this.SYNC_INTERVAL_MS / 1000 / 60} minutes`);
  ...
  async performSync() {
    try {
      ...
      // Analyze differences
      const analysis = this.analyzeDifferences(binanceData.pairs, databasePairs);
  ...
  /**
   * Fetch current trading pairs from Binance API
   */
  async fetchBinanceData() {
    try {
      ...
      console.log(`✅ Found ${filterResult.filteredPairs.length} active USDT pairs on Binance`);
  ...
  async getDatabasePairs() {
    try {
      const pairs = await Crypto.find({}).select('symbol status isSpotTradingAllowed').lean();
  ...
  analyzeDifferences(binancePairs, databasePairs) {
    ...
    const delistedPairs = databasePairs.filter(pair => !binanceSet.has(pair.symbol));
    const existingPairs = databasePairs.filter(pair => binanceSet.has(pair.symbol));
    ...
    if (newPairs.length > 0) {
      console.log(`\n🆕 NEW PAIRS:`, newPairs.slice(0, 10).join(', ') + (newPairs.length > 10 ? ` (+${newPairs.length - 10} more)` : ''));
    }
    
    if (delistedPairs.length > 0) {
      console.log(`\n🗑️ DELISTED PAIRS:`, delistedPairs.map(p => p.symbol).slice(0, 10).join(', ') + (delistedPairs.length > 10 ? ` (+${delistedPairs.length - 10} more)` : ''));
    }
  ...
  async applyChanges(analysis, binanceData) {
    ...
    try {
      ...
      if (analysis.newPairs.length > 0) {
        ...
        const addResult = await this.addNewPairs(analysis.newPairs, binanceData);
      ...
      if (analysis.delistedPairs.length > 0) {
        ...
        const removeResult = await this.removeDelistedPairs(analysis.delistedPairs);
      ...
      if (analysis.existingPairs.length > 0) {
        ...
        const updateResult = await this.updateExistingPairs(analysis.existingPairs, binanceData);
  ...
  async addNewPairs(newPairs, binanceData) {
    ...
    try {
      ...
      for (const symbol of newPairs) {
        try {
          const ticker = binanceData.tickerData.find(t => t.symbol === symbol);
      ...
      if (documentsToAdd.length > 0) {
        const insertResult = await Crypto.insertMany(documentsToAdd, { ordered: false });
  ...
  async removeDelistedPairs(delistedPairs) {
    ...
    try {
      ...
      if (symbolsToRemove.length > 0) {
        ...
        console.log(`✅ Removed ${results.removed} delisted pairs successfully`);
    ...
    try {
      ...
      for (const pair of existingPairs) {
        try {
          ...
          console.error(`❌ Error preparing update for ${pair.symbol}:`, error.message);
          results.errors++;
        }
      }
      ...
      if (bulkOps.length > 0) {
        ...
        console.log(`✅ Updated ${results.updated} existing pairs successfully`);
      }
