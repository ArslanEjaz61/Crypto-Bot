...
const getTriggeredAlertsBySymbol = async (req, res) => {
  try {
    ...
    const triggeredAlerts = await TriggeredAlert.getBySymbol(symbol, parseInt(limit));
...
const createTriggeredAlert = async (alertId, conditionMet, marketData, notificationDetails) => {
  try {
    ...
    console.log('notificationDetails:', JSON.stringify(notificationDetails, null, 2));
    ...
    console.log(`✅ Triggered alert saved successfully with ID: ${savedAlert._id}`);
    console.log(`📊 Triggered alert created for ${alert.symbol}: ${conditionMet.description}`);
    ...
    if (verification) {
      ...
      console.error(`❌ Verification failed: Triggered alert not found in database`);
    }
