...
alertSchema.methods.checkConditions = function (data) {
  ...
  if (this.alertCountEnabled && this.alertCountTimeframe) {
    console.log(
      `✅ Using timeframe-based continuous monitoring for ${this.alertCountTimeframe}`
    );
    ...
    if (this.lastTriggered) {
      ...
      console.log(
        `🔍 Minutes since last trigger: ${minutesSinceLastTrigger.toFixed(2)}`
      );

      if (minutesSinceLastTrigger < 30) {
        console.log(
          `❌ Alert not triggered: Too soon since last trigger (${minutesSinceLastTrigger.toFixed(
            2
          )} minutes < 30 minutes)`
        );
        console.log(
          `💡 TIP: Enable alert count system for continuous monitoring`
        );
        return false;
      }
    }
  ...
  if (this.targetType === "price") {
    ...
    if (this.direction === ">" && currentPrice >= this.targetValue) {
      console.log(
        `✅ Price condition met: ${currentPrice} >= ${this.targetValue}`
      );
      ...
      console.log(
        `❌ Price condition not met: ${currentPrice} ${this.direction} ${this.targetValue}`
      );
    }
  ...
  if (marketData) {
    ...
    if (
      ...
      console.log(
        `   Difference: ${marketData.dailyVolume - this.minDailyVolume}`
      );
      marketFiltersPass = false;
    ...
    for (const timeframe of this.candleTimeframes) {
      ...
      if (candleData) {
        ...
        if (timeframeConditionMet) {
          ...
          console.log(
            `🔍 Checking candle alert for ${this.symbol} ${timeframe}:`
          );
          ...
          if (!alertState || alertState.lastTriggeredCandle !== candleKey) {
            ...
            console.log(
              `🚨 CANDLE ALERT TRIGGERED: ${this.symbol} ${timeframe} ${this.candleCondition}`
            );
            console.log(
              `   ✅ First time triggering for this candle: ${candleKey}`
            );
            ...
            console.log(
              `⏳ Candle alert already triggered for ${this.symbol} ${timeframe} candle: ${candleKey}`
            );
            console.log(`   ❌ Skipping duplicate trigger`);
          }
        } else {
          console.log(
            `❌ Candle condition not met for ${this.symbol} ${timeframe} ${this.candleCondition}`
          );
        }
      }
    }
...
alertSchema.methods.resetCandleAlertStates = function (
  ...
  console.log(
    `🔄 Checking candle state reset for ${this.symbol} ${timeframe}:`
  );
  ...
  if (alertState && alertState.lastTriggeredCandle) {
    ...
    if (alertState.lastTriggeredCandle !== currentCandleKey) {
      console.log(
        `   ✅ New candle detected! Resetting alert state for ${timeframe}`
      );
      this.candleAlertStates.set(timeframe, {
        triggered: false,
...
alertSchema.methods.wasCandleAlertTriggered = function (
  ...
  console.log(
    `🔍 Checking if alert was triggered for ${this.symbol} ${timeframe}:`
  );
  console.log(`   Current candle key: ${currentCandleKey}`);
...
alertSchema.methods.isAlertCountLimitReached = function (
  timeframe,
  candleOpenTime
) {
  if (!this.alertCountEnabled || !this.alertCountTimeframe) {
  ...
  if (limitReached) {
    console.log(
      `   🚫 ALERT BLOCKED: ${this.symbol} has already sent ${counter.count} alerts in current ${timeframe} candle`
    );
    console.log(
      `   🚫 Reason: Alert count limit reached (${counter.count}/${this.maxAlertsPerTimeframe})`
    );
    console.log(`   🚫 Next alert allowed when new ${timeframe} candle opens`);
  } else {
    console.log(
      `   ✅ ALERT ALLOWED: ${this.symbol} can send alert (${counter.count}/${this.maxAlertsPerTimeframe})`
    );
  }
...
alertSchema.methods.enableContinuousMonitoring = function (
  timeframe = "5MIN",
  maxAlertsPerCandle = 1
) {
  ...
  console.log(
    `✅ Continuous monitoring enabled for ${this.symbol} on ${timeframe} timeframe`
  );
  console.log(`   Alert count enabled: ${this.alertCountEnabled}`);
  console.log(`   Alert count timeframe: ${this.alertCountTimeframe}`);
