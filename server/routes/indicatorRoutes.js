...
const { makeThrottledRequest } = require("../utils/requestThrottle");

// Helper function to get Binance klines data with throttling
async function getBinanceKlines(symbol, interval, limit = 500) {
  try {
    ...
    const response = await makeThrottledRequest(url, { params });
...
router.get("/:symbol/rsi", async (req, res) => {
  try {
    ...
    console.log(
      `Fetching RSI for ${symbol}, period: ${period}, timeframe: ${timeframe}`
    );
    ...
    if (!klines || klines.length === 0) {
      return res
        .status(404)
        .json({ error: "No data available for this symbol" });
    }
    ...
    if (rsi === null) {
      return res
        .status(400)
        .json({ error: "Insufficient data to calculate RSI" });
    }
...
router.get("/:symbol/ema", async (req, res) => {
  try {
    ...
    console.log(
      `Fetching EMA for ${symbol}, periods: ${periods}, timeframe: ${timeframe}`
    );
...
router.get("/:symbol/volume-history", async (req, res) => {
  try {
    ...
    console.log(
      `Fetching volume history for ${symbol}, limit: ${limit}, timeframe: ${timeframe}`
    );
...
router.get("/:symbol/ohlcv", async (req, res) => {
  try {
    ...
    console.log(
      `Fetching OHLCV for ${symbol}, timeframe: ${timeframe}, limit: ${limit}`
    );
...
router.get("/health", async (req, res) => {
  try {
    const { makeThrottledRequest } = require("../utils/requestThrottle");
    const { getRequestStats } = require("../utils/requestThrottle");

    // Test API connectivity with a simple request
    const testResponse = await makeThrottledRequest(
